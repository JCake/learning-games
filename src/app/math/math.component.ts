...
export class MathComponent implements OnInit {
  ...
  giveProblem(){
    ...
    this.givenAnswer = '';
    if(this.level < 5){
      this.configEasyAddition();
    } else if (this.level < 8){
      this.configEasyMinus();
    } else if (this.level < 15){
      this.configHarderAddition();
    } else if (this.level < 20){
      this.configMultiplication();
    } else {
      this.configHardest();
    }
  }

  private configEasyAddition() {
